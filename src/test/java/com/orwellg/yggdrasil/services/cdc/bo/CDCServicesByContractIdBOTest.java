...
public class CDCServicesByContractIdBOTest {
	
	protected CDCServicesByContractIdBO cdcServicesByContractIdBo;
	
	@Mock
	protected ServicesByContractIdRepositoryImpl servicesByContractIdDao;
	...
	public void setUp() {
		...
		cdcServicesByContractIdBo.LOG = LOG;
	}
	...
	public void testCDCInsert() {
		...
		// When process cr
		ServicesByContractId services = cdcServicesByContractIdBo.processChangeRecord(cr);

		// Then event data inserted in scylla repository
		verify(servicesByContractIdDao).insert(services);
		// And logged at info level ChangeRecord executed including the original ChangeRecord event as it came from maxscale.
		verify(cdcServicesByContractIdBo.LOG).info("ServicesByContractId inserted in scylla = {} for changeRecord event = {}", services, cr);
	}
	...
	public void testCDCUpdate() {
		{
			...
			// Then do nothing
			verify(servicesByContractIdDao, never()).insert(services);
			verify(servicesByContractIdDao).delete(services);
			// And logged at info level nothing done including the original ChangeRecord event as it came from maxscale.
			verify(cdcServicesByContractIdBo.LOG).info("ServicesByContractId deleted in scylla = {} for changeRecord event = {}", services, cr);
		}
